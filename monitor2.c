...
/* PEEPHOLE CTRL
 * bit 0: PAIR_ADDR_VALID
 * bit 1: PAIR_DATA_VALID
 * bit 8: MODE (0:PAIR,1:FREEDOM) 
 * */
#define NVC0_PBUS_PEEPHOLE_W_CTRL 0x00155c
#define NVC0_PBUS_PEEPHOLE_W_ADDR 0x060000
#define NVC0_PBUS_PEEPHOLE_W_DATA 0x060004
#define NVC0_PBUS_INTR_EN 0x001140
#define NVC0_PBUS_INTR 0x001100
#define NVC0_PBUS_INTR_PEEPHOLE_W_PAIR_MISMATCH 0x100//bit 12

void peephole_w_addr_op(int value){
    uint32_t peephole_ctrl=0;

    NV_WR32(NVC0_PBUS_PEEPHOLE_W_ADDR,value);
    peephole_ctrl = NV_RD32(NVC0_PBUS_PEEPHOLE_W_CTRL);
    if((peephole_ctrl|0x10) == 0){
	if((peephole_ctrl | 0x1) == 0x1){
	    NV_WR32(NVC0_PBUS_INTR_EN,NVC0_PBUS_INTR_PEEPHOLE_W_PAIR_MISMATCH);
	}else if(peephole_ctrl | 0x02){
	    //peepholewrite32
	    NV_WR32(NVC0_PBUS_PEEPHOLE_W_CTRL,(peephole_ctrl & 0xfffffffd));
	}else{
	    NV_WR32(NVC0_PBUS_PEEPHOLE_W_CTRL,peephole_ctrl | 0x01);
	}
	    
    }
}

void peephole_w_data_op(int value){
    uint32_t peephole_ctrl=0;
    NV_WR32(NVC0_PBUS_PEEPHOLE_W_ADDR,value);
    peephole_ctrl = NV_RD32(NVC0_PBUS_PEEPHOLE_W_CTRL);
    if((peephole_ctrl|0x10) == 0){
	if((peephole_ctrl | 0x2) == 0x2){
	    NV_WR32(NVC0_PBUS_INTR_EN,NVC0_PBUS_INTR_PEEPHOLE_W_PAIR_MISMATCH);
	}else if(peephole_ctrl | 0x1){
	    //peepholewrite32
	    //memory write byte enables copied from the MMIO access
	    NV_WR32(NVC0_PBUS_PEEPHOLE_W_CTRL,(peephole_ctrl & 0xffffffffe));
	}else{
	    NV_WR32(NVC0_PBUS_PEEPHOLE_W_CTRL,peephole_ctrl | 0x02);
	}
    }else{
	//peephole write_be(PEEPHOLE_W_ADDR,value,value_be);
    }
}
...
#endif
 NV_WR32(NVC0_PBUS_PEEPHOLE_W_CTRL,0x3);
 NV_WR32(NVC0_PBUS_PEEPHOLE_W_ADDR,0xf1234);
 NV_WR32(NVC0_PBUS_PEEPHOLE_W_DATA,0xff1234f);
  
 printf("[PEEPHOLE_W_CTRL]:0x%08x,[PEEPHOLE_W_ADDR]:0x%08x,[PEEPHOLE_W_DATA]:0x%08x\n",NV_RD32(NVC0_PBUS_PEEPHOLE_W_CTRL),
	 NV_RD32(NVC0_PBUS_PEEPHOLE_W_ADDR), NV_RD32(NVC0_PBUS_PEEPHOLE_W_DATA));
 while(1){
     printf("whats mmio register address?(when write to \"0\"): ");
     scanf("%x",&add);
     if(add==0){
     	printf("whats mmio register address?: ");
     	scanf("%x",&add);
	 printf("whats value?:");
	scanf("%x",&val_peep);
	 NV_WR32(add,val_peep);
